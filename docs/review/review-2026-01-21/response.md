...
- **依存関係**: 問題 2-1 が前提（Pure Data Structure 化により実装が簡単になる）
- **関連Issue/PR**:
- **備考**:
  - propagation なしなら双方向マッピングの同期ロジックがシンプル
  - Pure 化により `place` の複雑さがなくなるため、実装が容易
...
- **確認結果**:
  - 事実確認: 未確認（テストコードの調査が必要）
  - テストカバレッジ: 要調査
  - 実装の正当性: 要確認
- **対応方針**:
  - テストコードを調査し、カバレッジを確認
  - 複数解を持つパズルのテストケースを追加（必要であれば）
  - バックトラックの正当性を検証
